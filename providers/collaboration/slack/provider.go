...
func (p *provider) sendMessage(ctx context.Context, request connector.TypedRequest[SendMessageInput]) (connector.DeliveryResult, error) {
	...
	payload, err := notificationmessage.ResolveProviderPayload(ProviderKey, request.Input.NotificationContent, request.Input.ProviderPayload)
	if err != nil {
		return connector.DeliveryResult{}, permanent("notification_content_invalid", err.Error())
	}
	if len(payload) == 0 {
