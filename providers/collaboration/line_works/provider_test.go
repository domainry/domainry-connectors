...
func TestDescriptorAndEndpointBoundary(t *testing.T) {
	...
	for _, endpoint := range []string{"https://www.worksapis.com", "https://www.worksapis.com/v2", "https://www.worksapis.com/v1.0/custom", "https://worksapis.com/v1.0", "http://www.worksapis.com/v1.0"} {
		if err = validator.ValidateConfig(connection(endpoint)); err == nil {
			t.Fatalf("invalid accepted=%s", endpoint)
		}
	}
	invalid := connection(defaultAPIBase)
	invalid.Config["bot_id"] = ""
	if err = validator.ValidateConfig(invalid); err == nil {
		t.Fatal("missing bot accepted")
	}
}
func TestCallsKeepBotTokenRuntimeOnly(t *testing.T) {
	transport := &recordingTransport{responses: []connector.HTTPResponse{{StatusCode: 200, Body: []byte(`{"botId":"bot-1"}`)}, {StatusCode: 200, Body: []byte(`{"requestId":"request-1"}`)}}}
	...
	delivery, err := adapter.Call(t.Context(), call(SendMessage.Key, SendMessage.ContractSHA256, connector.ModeEnqueue, true, SendMessageInput{Recipient: "user/one", Message: "Approved"}))
	if err != nil || delivery.ResponseRef != "line_works:request-1" {
		t.Fatalf("delivery=%+v err=%v", delivery, err)
	}
	if transport.requests[0].URL != "http://localhost:8080/bots/bot-1" || transport.requests[1].URL != "http://localhost:8080/bots/bot-1/users/user%2Fone/messages" {
		t.Fatalf("requests=%+v", transport.requests)
	}
	for _, request := range transport.requests {
		if request.SecretHeaders["Authorization"][0] != "Bearer bot-secret" || strings.Contains(request.URL+string(request.Body), "bot-secret") {
			t.Fatalf("token boundary=%+v", request)
		}
	}
}
func TestPayloadValidationAndWriteClassification(t *testing.T) {
	transport := &recordingTransport{responses: []connector.HTTPResponse{{StatusCode: 200, Body: []byte(`{}`)}}}
	adapter, _ := New(transport)
	_, err := adapter.Call(t.Context(), call(SendMessage.Key, SendMessage.ContractSHA256, connector.ModeEnqueue, true, SendMessageInput{Recipient: "user", Message: "fallback", ProviderPayload: map[string]any{"content": map[string]any{"type": "image", "fileId": "file-1"}}}))
	if err != nil || !strings.Contains(string(transport.requests[0].Body), `"type":"image"`) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, callErr := current.Call(t.Context(), call(SendMessage.Key, SendMessage.ContractSHA256, connector.ModeEnqueue, true, SendMessageInput{Recipient: "user", Message: "hello"}))
...
func connection(endpoint string) connector.Connection {
	return connector.Connection{Config: map[string]any{"api_base_url": endpoint, "bot_id": "bot-1", "timeout_seconds": 15}}
}
func call(key, hash string, mode connector.OperationMode, delivery bool, payload any) connector.CallRequest {
	raw, _ := json.Marshal(payload)
	return connector.CallRequest{ConnectorKey: ConnectorKey, ProviderKey: ProviderKey, OperationKey: key, ContractSHA256: hash, Mode: mode, Delivery: delivery, Connection: connection("http://localhost:8080"), Secrets: map[string]string{"bot_token": "bot-secret"}, Payload: raw}
}
