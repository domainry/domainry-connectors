// Package surveymonkey implements the official SurveyMonkey feedback Provider.
...
const (
	...
	ProviderKey          = "surveymonkey"
	defaultBaseURL       = "https://api.surveymonkey.com/v3"
...
func New(transport connector.Transport) (connector.Adapter, error) {
	if transport == nil {
		return nil, errors.New("SurveyMonkey transport is required")
	}
...
func schema() connector.ProviderSchema {
	...
	return connector.ProviderSchema{ConnectorKey: ConnectorKey, ProviderKey: ProviderKey, ProviderRevision: "1.0.0", ConfigFields: []connector.ConfigField{{Key: "base_url", Name: "SurveyMonkey API v3 base URL", Type: connector.ConfigFieldText, Required: true, Default: json.RawMessage(`"https://api.surveymonkey.com/v3"`)}, {Key: "survey_id", Name: "Default survey ID", Type: connector.ConfigFieldText}, {Key: "timeout_seconds", Name: "Timeout seconds", Type: connector.ConfigFieldInteger, Default: json.RawMessage(`30`), Validation: connector.ConfigValidation{Min: &minimum, Max: &maximum}}}, SecretFields: []connector.SecretField{{Key: "access_token", Name: "OAuth access token", Required: true, CredentialKind: connector.SecretCredentialBearerToken, MaterialFormat: connector.SecretMaterialOpaque, RotationPolicy: connector.SecretRotationManual, ExpiryPolicy: connector.SecretExpiryOptional, TestRequirement: connector.SecretTestWhenBound}}}
}
func (p *provider) ValidateConfig(connection connector.Connection) error {
	...
	if err != nil || parsed.Host == "" || parsed.User != nil || (parsed.Scheme != "https" && !(parsed.Scheme == "http" && isLoopback(parsed.Hostname()))) {
		return connector.PermanentError("surveymonkey.endpoint_invalid", errors.New("HTTPS or loopback HTTP endpoint is required"))
	}
	return nil
}
func (p *provider) TestConnection(ctx context.Context, request connector.TestConnectionRequest) (connector.TestConnectionResult, error) {
	payload, ref, err := p.execute(ctx, request.Connection, request.Secrets, "/users/me", nil)
	if err != nil {
...
func (p *provider) getForm(ctx context.Context, request connector.TypedRequest[GetFormInput]) (connector.TypedResult[map[string]any], error) {
	id, err := surveyID(request.Connection, request.Input.FormID)
	if err != nil {
		return connector.TypedResult[map[string]any]{}, err
	}
	payload, ref, err := p.execute(ctx, request.Connection, request.Secrets, "/surveys/"+url.PathEscape(id)+"/details", nil)
...
func (p *provider) listResponses(ctx context.Context, request connector.TypedRequest[ListResponsesInput]) (connector.TypedResult[map[string]any], error) {
	...
	if pageSize < 1 || pageSize > 100 {
		return connector.TypedResult[map[string]any]{}, connector.PermanentError("surveymonkey.page_size_invalid", errors.New("page size must be between 1 and 100"))
	}
	query := url.Values{"per_page": {strconv.Itoa(pageSize)}}
	if value := strings.TrimSpace(request.Input.Since); value != "" {
		query.Set("start_created_at", value)
	}
	if value := strings.TrimSpace(request.Input.Until); value != "" {
		query.Set("end_created_at", value)
	}
	if value := strings.TrimSpace(request.Input.After); value != "" {
		query.Set("page", value)
	}
	payload, ref, err := p.execute(ctx, request.Connection, request.Secrets, "/surveys/"+url.PathEscape(id)+"/responses/bulk", query)
	if err == nil {
...
func (p *provider) execute(ctx context.Context, connection connector.Connection, secrets map[string]string, path string, query url.Values) (map[string]any, string, error) {
	...
	if token == "" {
		return nil, "", connector.PermanentError("surveymonkey.access_token_required", errors.New("resolved access token is required"))
	}
	endpoint, err := url.Parse(baseURL(connection) + path)
	if err != nil {
		return nil, "", connector.PermanentError("surveymonkey.request_invalid", err)
	}
	...
	if err != nil {
		return nil, "", connector.RetryableError("surveymonkey.network_error", err)
	}
	ref := "http:" + strconv.Itoa(response.StatusCode)
	payload := map[string]any{}
	if json.Unmarshal(response.Body, &payload) != nil {
		return nil, ref, connector.PermanentError("surveymonkey.response_invalid", errors.New("provider response is invalid JSON"))
	}
...
func surveyID(connection connector.Connection, input string) (string, error) {
	...
	return "", connector.PermanentError("surveymonkey.survey_id_required", errors.New("survey ID is required"))
}
...
func errorCode(payload map[string]any, status int) string {
	if object, ok := payload["error"].(map[string]any); ok {
		if id := configString(object, "id"); id != "" {
			return "surveymonkey.provider_" + codeToken(id)
		}
	}
	return "surveymonkey.http_" + strconv.Itoa(status)
}
